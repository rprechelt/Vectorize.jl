...
if isfile("$(pkgdir)deps\\downloads\\yeppp-1.0.0.tar.bz2") || (Libdl.find_library(["libyeppp"]) != "")
    ...
    trycmd(`$exe7z x $(pkgdir)deps\\downloads\\yeppp-1.0.0.tar.bz2 -o$(pkgdir)deps\\downloads\\`, err="Unable to decompress tar archive")
    trycmd(`$exe7z x $(pkgdir)deps\\downloads\\yeppp-1.0.0.tar -o$(pkgdir)deps\\src\\`, err="Unable to expand tar archive")
